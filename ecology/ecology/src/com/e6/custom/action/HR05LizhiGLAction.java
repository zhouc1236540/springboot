 ...
 *	人力资源节点提交后将工资结算截止日期更新HR离职日期
 */
public class HR05LizhiGLAction extends AbstractAction{
	
	private static Logger LOGGER = Logger.getLogger(HR05LizhiGLAction.class);
	
	@Override
	public void execute() throws Exception {
		Integer requestId = getRequestId();
		LOGGER.error("HR05员工离职办理流程人力资源节点---甘露 =="+requestId);
		JSONObject jsonObject = new JSONObject();
		setData(jsonObject);
		JSONObject sendPackaging = HttpUtils.sendUpdatePackaging(ConstantUtil.dongbaoLZ021001,jsonObject,requestId+"");
		LOGGER.error("HR05员工离职办理流程人力资源节点---甘露人员信息============="+sendPackaging.toString());
		...
		if(!value.equals("")) {
			setRequest("异常信息如下：","HR05员工离职办理流程---甘露 人力资源节点异常:"+value,getRequestInfo());
			throw new Exception("HR05LizhiGLAction");
		}
		LOGGER.error("HR05员工离职办理流程人力资源节点---甘露 结束========================");
	}
