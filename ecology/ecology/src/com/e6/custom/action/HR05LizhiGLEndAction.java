 ...
 *
 */
public class HR05LizhiGLEndAction extends AbstractAction{
	
	private static Logger LOGGER = Logger.getLogger(HR05LizhiGLEndAction.class);
	
	@Override
	public void execute() throws Exception {
		Integer requestId = getRequestId();
		LOGGER.error("HR05员工离职办理流程---甘露 =="+requestId);
		JSONObject jsonObject = new JSONObject();
		setData(jsonObject);
		JSONObject sendPackaging = HttpUtils.sendUpdatePackaging(ConstantUtil.dongbaoRs020101,jsonObject,requestId+"");
		LOGGER.error("HR05员工离职办理流程---甘露人员信息============="+sendPackaging.toString());
		...
		if(!value.equals("")) {
			setRequest("异常信息如下：","HR05员工离职办理流程---甘露 归档异常:"+value,getRequestInfo());
			throw new Exception("HR05LizhiGLEndAction");
		}
		LOGGER.error("HR05员工离职办理流程---甘露归档结束  ========================");
	}
